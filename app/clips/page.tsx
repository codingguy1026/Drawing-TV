...
const clips = [
  ...
  { id: 2, code: "S-028", title: "3초 뒤에 분위기 바뀜", channel: "404 플레이룸", handle: "@404play", likes: "41K", comments: "892", icon: "👁", tag: "GAME", gradient: "linear-gradient(165deg,#6675ff 0%,#34205d 50%,#0e0d15 100%)", ratio: [4, 5], ratioLabel: "4:5", frameMax: "500px" },
  { id: 3, code: "S-041", title: "선 하나로 그림 살리는 법", channel: "모노 드로잉", handle: "@mono", likes: "120K", comments: "2.1K", icon: "✎", tag: "ART", gradient: "linear-gradient(165deg,#ffd09f 0%,#9a63cb 52%,#151124 100%)", ratio: [1, 1], ratioLabel: "1:1", frameMax: "560px" },
  { id: 4, code: "S-052", title: "키보드 소리만 듣고 맞히기", channel: "D Tech", handle: "@dtech", likes: "67K", comments: "718", icon: "⌨", tag: "TECH", gradient: "linear-gradient(165deg,#41d5ba 0%,#15506b 50%,#0c1116 100%)", ratio: [16, 9], ratioLabel: "16:9", frameMax: "650px" },
  { id: 5, code: "S-063", title: "오늘 하늘 진짜 미쳤다", channel: "오늘의 우리", handle: "@todayus", likes: "98K", comments: "1.8K", icon: "☀", tag: "DAILY", gradient: "linear-gradient(165deg,#6dc1ff 0%,#6557a4 52%,#171426 100%)", ratio: [9, 16], ratioLabel: "9:16", frameMax: "420px" },
];
...
export default function ClipsPage() {
  ...
  const frameVars = {
    "--frame-max": current.frameMax,
    "--ratio-w": current.ratio[0],
    "--ratio-h": current.ratio[1],
  } as CSSProperties;
  ...
  return (
    <main className="clip-shell">
      ...
      <section className="clip-stage">
          ...
          <div className="queue-list">
            {clips.map((clip, clipIndex) => (
              <button key={clip.id} className={clipIndex === index ? "active" : ""} onClick={() => { setIndex(clipIndex); setCommentsOpen(false); }}>
                <i className="queue-thumb" style={{ "--thumb": clip.gradient, "--thumb-w": clip.ratio[0], "--thumb-h": clip.ratio[1] } as CSSProperties}>{clip.icon}</i>
                <span><small>{clip.code} · {clip.tag} · {clip.ratioLabel}</small><b>{clip.title}</b><em>{clip.handle}</em></span>
        ...
        <section className="signal-viewer" style={frameVars}>
          <div className="viewer-frame" style={{ "--signal-bg": current.gradient } as CSSProperties}>
            ...
            <div className="viewer-copy">
              ...
              <h1>{current.title}</h1>
              <p>{current.handle} · 원본 {current.ratioLabel} 비율로 재생 중</p>
            </div>

            <div className="viewer-meter"><span style={{ width: `${progress}%` }} /></div>
            <div className="viewer-corners"><i>REC</i><i>{current.ratioLabel}</i></div>
          </div>
        ...
        <aside className={`comment-drawer ${commentsOpen ? "open" : ""}`}>
          ...
          <div className="comments">
            ...
            <p><b>@mono</b><span>영상 비율 따라 크기 바뀌는 게 맞지 📺</span><small>1분 전</small></p>
          </div>
      ...
      <nav className="mobile-signal-nav">
        ...
        <button className="current"><b>{current.code}</b><span>{current.ratioLabel} · {String(index + 1).padStart(2, "0")} / {String(clips.length).padStart(2, "0")}</span></button>
