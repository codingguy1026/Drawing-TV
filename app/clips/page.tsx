...
export default function ClipsPage() {
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if (event.key === "ArrowDown" || event.key === "j") tune(1);
      if (event.key === "ArrowUp" || event.key === "k") tune(-1);
    ...
    <main className="clip-shell">
      ...
      <section className="clip-stage">
        <aside className="signal-queue" aria-label="클립 신호 목록">
          ...
          <div className="queue-tip"><kbd>J</kbd><kbd>K</kbd><span>키로 신호 전환</span></div>
        </aside>
