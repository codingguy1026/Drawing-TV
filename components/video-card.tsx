          ...
          >
            ...
            {video.progress !== undefined && (
              <div className="absolute inset-x-2 bottom-2 h-1.5 overflow-hidden rounded-full bg-white/20">
                <div className="h-full rounded-full bg-[var(--accent)]" style={{ width: `${video.progress}%` }} />
              </div>
            )}
            <div className="absolute right-3 top-3 rounded-full bg-black/60 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-white">{video.duration}</div>
            <div className="absolute left-3 top-3 rounded-full border border-white/30 bg-black/30 px-2 py-1 text-[10px] font-semibold text-white">{video.category}</div>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-2">
            <Link href={`/watch/${video.id}`} className="min-w-0 flex-1">
              <h3 className="line-clamp-2 text-sm font-semibold leading-5 text-[var(--foreground)] hover:text-[var(--accent)]">{video.title}</h3>
            </Link>
            {video.badge && (
              <span className="mt-0.5 rounded-full border border-[var(--accent)]/30 bg-[var(--accent-soft)] px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--accent)]">{video.badge}</span>
            )}
          </div>
          ...
          <div className="mt-1 flex items-center gap-2 text-[11px] text-[var(--muted)]">
            <span>{video.views} 조회</span><span>•</span><span>{video.published}</span>
          </div>
        </div>
      </div>
    </article>
